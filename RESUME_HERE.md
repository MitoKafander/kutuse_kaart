...
- **Vald-boundary "double lines" are a DATA problem, not styling.** `maakonnad.geojson` + `parishes.geojson` shipped as independent geometries at ~2–3-decimal precision, so county and vald edges drew offset with gaps. **County layer FIXED 2026-07-21:** `scripts/rebuild_maakonnad.mjs` regenerates `maakonnad.geojson` as the dissolved union of the valds (`-dissolve maakond_id`) → county-edge coincidence 11.8%→88.6%; `parishes.geojson` left untouched (snapping it collapses ~2000 coastal islets). Residual *vald-vald* doubling (~26% shared arcs) still needs topological re-source — not done.
