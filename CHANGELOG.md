...
- Verified via a 4-lens adversarial workflow (DB integrity, geometry/data-loss, app-code compatibility, regression) — all pass. Also confirmed **live on kyts.ee**: at field-level zoom the vald border draws as a single dashed line coinciding with the OSM basemap boundary (no doubling / no-man's-land), 0 console errors.
...
### Fixed 🐛
- 🟡 **`vercel.json` rewrite served `index.html` for missing `/assets/*.js`** (`vercel.json`): the catch-all rewrite `"source": "/(.*)"` was matching paths under `/assets/` that didn't resolve to a real file, returning the SPA shell HTML with `Content-Type: text/html`. Browsers tried to parse that HTML as JavaScript and threw `is not a valid JavaScript MIME type` / `error loading dynamically imported module` — the symptom behind Sentry KYTS-WEB-P (StationDrawer chunk load) and the same fingerprint Safari surfaces when a stale tab requests a hashed chunk that's no longer on the CDN. Confirmed with `curl -sI https://kyts.ee/assets/ManualPriceModal-OLDHASH.js` returning HTTP 200 + `content-type: text/html`. Source narrowed to `"/((?!api/|assets/).*)"` so the rewrite only catches SPA route paths; missing assets and missing API routes now 404 properly. Side effect: `App.tsx`'s `lazyWithReload` chunk-error catch (`/Failed to fetch dynamically imported module|is not a valid JavaScript MIME type/`) will fire on a real 404 instead of a content-type mismatch — same outcome, cleaner error trail in Sentry.
- 🟡 **Stale-bundle backstop in `main.tsx`** (`src/main.tsx:87-118`): `lazyWithReload` only fires when a *missing* chunk hash 404s; it doesn't help when a service worker holds BOTH a stale `index.html` and the matching stale chunk hashes — every dynamic import succeeds and the user runs old code indefinitely. Observed 2026-05-01 12:23-12:35 EE: a "first visit" Chrome iOS user (distinct_id `019de2d9-3c60-750c-b002-ebf86f24c155`) tried to save Terminal Oil Mustakivi prices, hit the legacy hardcoded `alert("Viga hinna salvestamisel!")` (removed 2026-04-18 in `15d2938`), and her PostHog `ai_scan_success` events lacked the `attempts_made` / `model_used` props that landed 2026-04-22 in `be048e2` — running 9+ day old code despite the live bundle being current. Fix is `detectStaleBundle()` scheduled at idle: read the bundle hash from the page's `<script type="module" src="/assets/index-*.js">`, fetch `/index.html` with `cache: 'reload'`, compare. On mismatch: set `sessionStorage['kyts:bundle-reloaded']` to gate against reload loops, unregister every service-worker registration on the origin, then `window.location.reload()`. Next load fetches index.html fresh from Vercel (cache-control says `max-age=0, must-revalidate`) and the etag returns the latest, so the browser updates its HTTP cache and pulls the new chunks — no manual hard-refresh required from the user. Offline / network-blocked / no-current-script edge cases all early-return without action.

### Key Decisions
- **vercel.json regex `(?!api/|assets/)` instead of file-extension lookahead**: the cleaner pattern would have been "rewrite paths without a file extension" (e.g. `/((?!.*\\.).*)`) but that breaks legitimate SPA routes that happen to contain dots (`station/Olerex_2.0` etc.). Explicit prefix exclusion is more conservative — catches the actual problem (`/assets/X.js` and `/api/X` masking) and leaves all other path shapes untouched.
- **`detectStaleBundle` runs at idle, not at boot**: putting it on the critical path would burn an extra `/index.html` round-trip on every cold load. Scheduling it via `requestIdleCallback` (same scheduler as the existing `registerSW` call) keeps it off LCP and lets first-paint complete normally; the reload only matters if you're stuck running old code, and one extra second of stale code before reload is invisible compared to the multi-day wedge it's escaping.
- **Don't `caches.delete()` after unregister**: the service worker is the only thing serving from precache. Once unregistered, the browser HTTP cache for `index.html` is `max-age=0, must-revalidate`, so the next reload revalidates with Vercel and gets fresh content + new chunk hashes — explicit cache wipe adds blast radius without benefit and could mask future failures by hiding their root cause.
- **`sessionStorage` gate on the auto-reload, not `localStorage`**: a stale bundle stuck in `sessionStorage` clears when the user closes the tab, so they get a fresh shot on next visit. `localStorage` would persist the gate forever and silently keep them on stale code if the first reload didn't take. Same pattern `lazyWithReload` already uses (`kyts:chunk-reloaded`) — keep the failure modes adjacent and predictable.

### Open Items
- **Today's reporter (Terminal Oil Mustakivi)**: she's still on whatever stale code her browser cached. Once this ships and she revisits, `detectStaleBundle` will trigger one auto-reload and she'll be on current code. Any subsequent save failure will surface a real Estonian error message + capture to Sentry KYTS-WEB-N so we can see the root cause. No way to push the fix to her current cached session — only the next visit benefits.
- **Watch Sentry KYTS-WEB-P count after deploy**: the chunk-load error rate should drop to near-zero once stale tabs auto-recover. If it stays high, there's a second cache layer (CDN edge, ISP proxy) we haven't accounted for.
- **`detectStaleBundle` doesn't currently telemetry the reload**: if the auto-reload starts firing for legitimate (transient) reasons, we'd have no visibility. Easy follow-up: `captureReloadSafe('stale_bundle_auto_reload', { from: currentHash, to: freshHash })` before the reload kicks in, and watch the rate in PostHog.
