...
### Copy Fixes (same day)
- 🟢 **Card 2 no longer implies you must select a fuel type first**: the yellow FAB opens CheapestNearby which lists 95/98/D/LPG cheapest all at once — no pre-selection required. Rewrote body to reflect the actual behavior.
- 🟢 **"Jäta vahele" word order fixed** (was "Vahele jäta").
- 🟢 **New 6th card: "Loo konto"** — call-to-action explaining that login unlocks price submission, voting, favorites, leaderboard, loyalty discounts and cross-device sync. Previously login benefits were not mentioned anywhere in the tour, which buried the retention hook.
