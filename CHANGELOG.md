...
- **Didn't handle `auth_leaked_password_protection` in SQL**: it's a Supabase Auth dashboard toggle, not a database change. Flip it at Dashboard → Authentication → Providers → Email → "Password requirements" → enable HaveIBeenPwned check.

### Open Items
- **Run the migration + dashboard toggle**: after applying phase 48 in the Supabase SQL Editor, enable the HaveIBeenPwned check in the Auth dashboard. Then reload Advisors → expect zero ERROR, zero WARN, and only the `rls_enabled_no_policy` INFO on `market_insight_runs` (intentional).
...
- **If the HaveIBeenPwned toggle breaks sign-ups**: some existing leaked passwords in the wild might block re-logins. Supabase's implementation only checks on *new* password set (sign-up + reset), not on login, so existing users aren't locked out. Worth knowing if anyone reports a sign-up failure right after the toggle.
