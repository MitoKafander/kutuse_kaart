...
function buildPrompt(input: TranslatorInput): string {
  ...
  return [
    ...
    '9. Signal-code glossary — the driver-practical meaning of each code.',
    '   Your prose MUST match this framing; do NOT imply the driver should',
    '   delay a fill-up unless the signal is "wait".',
    '   - buy_now: Wholesale has risen and pump prices have not caught up yet.',
    '       Recommend filling up now, before the rise lands at the pump.',
    '   - hold: Pump is already tracking wholesale — no timing edge either way.',
    '       Tell the driver they can fill up whenever they need to. Do NOT',
    '       say "wait" or "hold off"; the user label for this state is',
    '       "OK to buy" / "võid osta" / "можно заправиться" — match that tone.',
    '   - wait: Wholesale has dropped and pump is still high. Suggest',
    '       delaying the fill-up by 3–5 days if possible, since a dip is likely.',
    '   - neutral: Not enough data (few Kyts reports or a truly flat market).',
    '       Describe it as too quiet to call; do not recommend either way.',
    '',
    'INPUT:',
