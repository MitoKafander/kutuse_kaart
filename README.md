...
2. **AI Vision Scanning**: Open your device's camera, snap a photo of a gas station totem, and local Vercel nodes pass the image securely to **Gemini 2.5 Flash** to automatically read and hydrate the form pricing — with automatic brand cross-validation.
3. **User Profile & Favorites**: Logged-in users get a personal dashboard with favorite stations (showing live prices), a contribution score, and a saved fuel type preference that auto-filters the map on every launch.
4. **Advanced Filtering**: 
   - **Traffic Light System**: Instantly hide prices that are over 24 hours old so you only see active, verified data.
   - **Cheapest Finder**: Select a fuel type, and the app mathematically highlights the absolute cheapest station across the visible map with a glowing gold marker.
5. **Reputation & Voting**: Users must securely log in to submit a price or vote (Thumbs Up / Thumbs Down) on existing data. A strict one-vote-per-user database constraint ensures community integrity.
6. **Geolocation**: One-tap locator passively traces your device's GPS hardware for instant 0-second re-centering.
7. **GDPR Compliance**: Cookie consent banner and full Estonian-language privacy policy built in.
...
### Database Schema
The project schema relies on five tables governed by Row Level Security (RLS). Migrations are in `schema.sql`, `schema_phase3.sql`, and `schema_phase8.sql`.
...
*   `user_profiles` (Default fuel type preference per user)
